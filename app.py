...
if not df_liberi_base.empty:
    ...
    if giocatori_liberi:
        ...
        col4.metric(
            "🏷️ Prezzo Partenza (Quotazione Listone)", f"{quotazione_listone} cr"
        )
        ...
        with st.expander("📊 Metriche Avanzate & Partite (Titolare / Subentrato)"):
            ...
            c_m3.metric("Indice di Continuità", f"{safe_get(g_data, 'Indice_Continuita', 'N/D')}")

            c_m4, c_m5, c_m6 = st.columns(3)
            c_m4.metric("xG 90'", f"{safe_get(g_data, 'xG_90', 0.0)}")
            c_m5.metric("xA 90'", f"{safe_get(g_data, 'xA_90', 0.0)}")
            c_m6.metric("Rischio Infortunio", f"{safe_get(g_data, 'Rischio_Infortunio', 'Basso')}")

        # Form di assegnazione giocatore
        with st.form("form_assegnazione_asta"):
            st.markdown("### Assegna Giocatore")
            col_vinci1, col_vinci2 = st.columns(2)
            with col_vinci1:
                acquirente_scelto = st.selectbox("Assegna a:", PARTECIPANTI_LEGA, key="form_acquirente_sel")
            with col_vinci2:
                prezzo_aggiudicazione = st.number_input("Prezzo di Aggiudicazione (cr):", min_value=1, value=max(1, quotazione_listone), step=1, key="form_prezzo_asta")
            
            scadenza_default_form = str(g_data.get("Scadenza_Contratto", "Giugno 2030"))
            scadenza_scelta = st.text_input("Scadenza Contratto Assegnato:", value=scadenza_default_form, key="form_scadenza_input")

            submit_assegnazione = st.form_submit_button("✅ Conferma e Assegna Giocatore")
            if submit_assegnazione:
                item_aggiunto = {
                ...
                st.session_state.rose_lega[acquirente_scelto].append(item_aggiunto)
                
                idx_g = df[df["Nome"] == giocatore_sel].index[0]
                st.session_state.df_giocatori.at[idx_g, "Stato"] = acquirente_scelto
                st.session_state.df_giocatori.at[idx_g, "Scadenza_Contratto"] = str(scadenza_scelta)
                
                st.success(f"🎉 **{giocatore_sel}** assegnato a **{acquirente_scelto}** per **{prezzo_aggiudicazione} cr** (Contratto: {scadenza_scelta})!")
                st.rerun()
    else:
        st.info("Nessun giocatore disponibile con i filtri selezionati.")
else:
    st.success("Tutti i giocatori sono stati assegnati! Controlla le rose nella barra laterale.")