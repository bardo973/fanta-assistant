...
def get_stats_giocatore(nome):
    """Restituisce dict con le stats del giocatore se presenti"""
    if st.session_state.stats_storiche.empty or "Nome" not in st.session_state.stats_storiche.columns:
        return None
    df_g = st.session_state.stats_storiche[st.session_state.stats_storiche["Nome"].str.lower() == nome.lower()]
    if df_g.empty:
        return None
    # Cerca stagione 2025/26 o 2025-26
    stagione_mask = df_g["Stagione"].astype(str).str.contains("2025", case=False, na=False)
    if stagione_mask.any():
        return df_g[stagione_mask].iloc[0].to_dict()
    # Altrimenti prendi l'ultima disponibile
    return df_g.iloc[0].to_dict()

def is_scadenza_prossima(mese, anno):
    """Verifica se la scadenza e entro 6 mesi"""
    ...
    try:
        ...
        diff = (a - ANNO_CORRENTE) * 12 + (m - MESE_CORRENTE)
        return 0 <= diff <= 6
    except:
        return False
...
def analisi_scambio(oggetti1, oggetti2, d1, d2):
    ...
    return {"sq1": s1, "sq2": s2, "delta_quot": delta_quot, "delta_fm": delta_fm, "d1": d1, "d2": d2}

def get_prestiti_dati(squadra):
    """Restituisce lista giocatori dati in prestito da una squadra"""
    return [p for p in st.session_state.prestiti if p["Da"] == squadra]

def get_prestiti_ricevuti(squadra):
    """Restituisce lista giocatori ricevuti in prestito da una squadra"""
    return [p for p in st.session_state.prestiti if p["A"] == squadra]
...
st.sidebar.markdown("---")
st.sidebar.subheader("📤 Esporta / 📥 Importa Backup")
with st.sidebar.expander("💾 Backup file"):
    # Esporta
    export_data = {
        "squadre": st.session_state.squadre,
        "storico_mercato": st.session_state.storico_mercato,
    ...
    st.markdown("---")
    # Importa
    up_backup = st.file_uploader("Carica backup JSON", type=["json"], key="uback")
    if up_backup is not None:
        try:
            backup_data = json.loads(up_backup.read().decode('utf-8'))
            st.session_state.squadre = backup_data.get("squadre", {})
            st.session_state.storico_mercato = backup_data.get("storico_mercato", [])
            st.session_state.watchlist = backup_data.get("watchlist", [])
            st.session_state.prestiti = backup_data.get("prestiti", [])
            st.session_state.contratti = backup_data.get("contratti", {})
            db = backup_data.get("giocatori_db", [])
            st.session_state.giocatori_db = pd.DataFrame(db) if db else pd.DataFrame(LISTONE_DEFAULT)
            stats = backup_data.get("stats_storiche", [])
            st.session_state.stats_storiche = pd.DataFrame(stats) if stats else pd.DataFrame()
            for sq in NOMI_SQUADRE:
                if sq not in st.session_state.squadre:
                    st.session_state.squadre[sq] = {"crediti": 500, "rosa": []}
        ...
        if db.empty:
            ...
            if len(svinc) > 0:
                ...
                if rosa_stesso_ruolo:
                    ...
                    # Recupera stats
                    stats_acq = get_stats_giocatore(g_sel)
                    stats_rosa = get_stats_giocatore(g_par)

                    comp_data = {
                        "Metrica": ["Nome", "Ruolo", "Quotazione", "FantaMedia", "Costo Acquisto", "Indice Affare"],
                        "Da Acquistare": [
                            g_sel, ruolo_sel, int(info["Quotazione"]), info["FantaMedia"],
                            prezzo, round(info["FantaMedia"] / max(int(info["Quotazione"]), 1), 2)
                        ],
                        "In Rosa": [
                            g_par, g_par_obj.get("Ruolo", ruolo_sel), g_par_obj.get("Quotazione", "N/D"),
                            g_par_obj.get("FantaMedia", "N/D"), g_par_obj.get("Costo_Acquisto", "N/D"),
                            round(g_par_obj.get("FantaMedia", 0) / max(g_par_obj.get("Quotazione", 1), 1), 2) if g_par_obj.get("Quotazione") else "N/D"
                        ]
                    }

                    # Aggiungi stats se disponibili
                    if stats_acq or stats_rosa:
                        comp_data["Metrica"].extend(["Stats Gol", "Stats Assist", "Stats FM"])
                        comp_data["Da Acquistare"].extend([
                            stats_acq.get("Gol", "N/D") if stats_acq else "N/D",
                            stats_acq.get("Assist", "N/D") if stats_acq else "N/D",
                            stats_acq.get("FantaMedia", "N/D") if stats_acq else "N/D"
                        ])
                        comp_data["In Rosa"].extend([
                            stats_rosa.get("Gol", "N/D") if stats_rosa else "N/D",
                            stats_rosa.get("Assist", "N/D") if stats_rosa else "N/D",
                            stats_rosa.get("FantaMedia", "N/D") if stats_rosa else "N/D"
                        ])
        ...
        with col_a:
            ...
            if oggetti1_sel:
                for g in oggetti1_sel:
                    ...
                    stats_g = get_stats_giocatore(g['Nome'])
                    if stats_g:
                        st.caption(f"   📊 Stats: {stats_g.get('Gol','?')} gol, {stats_g.get('Assist','?')} assist, FM {stats_g.get('FantaMedia','?')}")
            else:
