...
if not df_liberi_base.empty:
    ...
    if giocatori_liberi:
        ...
        with st.expander("📊 Metriche Avanzate & Partite (Titolare / Subentrato)"):
            ...
            c_m2.metric("Partite da Subentrato Attese", f"{p_subentrato}")
            c_m3.metric("Indice di Continuità", f"{safe_get(g_data, 'Indice_Continuita', 7.0)} / 10")
            
            c_m4, c_m5, c_m6 = st.columns(3)
            c_m4.metric("Rischio Infortunio", f"{safe_get(g_data, 'Rischio_Infortunio', 'Medio')}")
            c_m5.metric("Rischio Turnover", f"{safe_get(g_data, 'Rischio_Turnover', 'Medio')}")
            c_m6.metric("Indice Cartellini", f"{safe_get(g_data, 'Indice_Cartellini', 'Medio')}")